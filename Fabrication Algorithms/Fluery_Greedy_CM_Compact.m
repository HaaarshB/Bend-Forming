function epath = Fluery_Greedy_CM_Compact(g,pos)
...
% Follows these website: https://slaystudy.com/fleurys-algorithm/, https://www.geeksforgeeks.org/fleurys-algorithm-for-printing-eulerian-path/
    ...
    if sum(mod(degree(g),2))==0 % all even degree nodes
        % [~,startnode] = min(vecnorm(pos,2,2)); % for starting node closest to origin
        startnode = randsample(numnodes(g),1); % for random sample
        epath = startnode;
    ...
    while length(epath)~=(numedges(galledges)+1)
        ...
        elseif ~stuckbool
            ...
            allneighbors = neighbors(g,u); % pick neighbor node with SHORTEST distance to geometric centroid of points already in the path
