function epath = Hierholzer_Greedy_CM(g,pos)
    ...
    while ~isempty(cpath)
        ...
        if isempty(neighbors(g,u))
            ...
            if length(neighbors(g,u))==1
                ...
                currentpathnodes = pos(cpath,:);
                previousnodepos = [mean(currentpathnodes(:,1)),mean(currentpathnodes(:,2)),mean(currentpathnodes(:,3))]; % center of mass of current Euler path
