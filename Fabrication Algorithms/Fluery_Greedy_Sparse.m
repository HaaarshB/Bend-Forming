function epath = Fluery_Greedy_Sparse(g,pos)
...
% Follows these website: https://slaystudy.com/fleurys-algorithm/, https://www.geeksforgeeks.org/fleurys-algorithm-for-printing-eulerian-path/
    ...
    while length(epath)~=(numedges(galledges)+1)
        ...
        elseif ~stuckbool
            ...
            allneighbors = neighbors(g,u); % pick neighbor node with FURTHEST distance to last node in Euler path
            ...
            [~,indx] = maxk(allneighborsdistancessorted(:,2),mindistanceneighborsnum); % THIS LINE IS DIFFERENT BETWEEN COMPACT AND SPARSE
            mindistanceneighbors = allneighborsdistancessorted(indx,1);
            if length(mindistanceneighbors)==1
                v = mindistanceneighbors;
            else
                v = randsample(mindistanceneighbors,1); % randomly pick neighbor with maximum distance
            end
