function plotbendpathwSC(geuler,eulerpath,pos,pausetime,savevideo,framerate,filename,vidangle)
%% Function which plots bend path on truss graph
    ...
    figure()
    pathplot = plot(geuler, 'XData', pos(:,1), 'YData', pos(:,2), 'Zdata', pos(:,3), 'NodeLabel', nodelabel, 'EdgeColor','k', 'LineWidth', 1, 'NodeColor','k');
    axis equal
    if savevideo
        set(gcf,'color','w');
        view(vidangle)
    ...
    highlight(pathplot,eulerpath(1),'MarkerSize',10,'NodeColor','g')
    hold on
    ...
    for i=1:(length(eulerpath)-1)
        highlight(pathplot,eulerpath(1),'MarkerSize',5,'NodeColor','r')
        % delete(CMplot)
        nodein = eulerpath(i);
        ...
        if ~ismember(edgepath,edgeindx(1))
            highlight(pathplot,'Edges',edgeindx(1),'LineWidth',1.5)
            highlight(pathplot,'Edges',edgeindx(1),'EdgeColor','r')
            highlight(pathplot,eulerpath(i+1),'MarkerSize',10,'NodeColor','g')
            edgepath(i) = edgeindx(1);
        else
            highlight(pathplot,'Edges',edgeindx(2),'LineWidth',1.5)
            highlight(pathplot,'Edges',edgeindx(2),'EdgeColor','r')
            highlight(pathplot,eulerpath(i+1))
            highlight(pathplot,eulerpath(i+1),'MarkerSize',10,'NodeColor','g')
        ...
        if savevideo
            frame = getframe;
            writeVideo(writerObj3D,frame);
            highlight(pathplot,eulerpath(i+1),'MarkerSize',5,'NodeColor','r')
        end
    end
end