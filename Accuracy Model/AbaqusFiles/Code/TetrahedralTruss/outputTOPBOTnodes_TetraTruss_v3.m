function outputTOPBOTnodes_TetraTruss_v3(perfnodes,impnodes,filename)
...
if sameplanenodecount >= (round(size(perfnodes,1)/2,TieBreaker='tozero')-3) % if more than half of the nodes are on the same plane (with some tolerance)
    ...
    while breakbool ~= 1 % want to find normal vector such that half of the furthest nodes have the same z-value, i.e. are on the same plane
        ...
        if norm(point2-point1) > referencelength && norm(point3-point1) > referencelength && norm(point3-point2) > referencelength % ensure all three nodes are far away from each other
            ...
            if sameplanenodecount >= round(size(perfnodes,1)/2,TieBreaker='tozero')-1 % if half of the nodes are on the same plane (with some tolerance)
                ...
                topplanenodes = impnodes(topplanenodesindx,:); % find all imperfect nodes on top plane
                bottomplanenodes = impnodes(bottomplanenodesindx,:); % find all imperfect nodes on bottom plane
                % Verify that top and bottom plane nodes are actually on two separate planes
                topplanenodesvaried = perfnodesvaried(topplanenodesindx,:);
                bottomplanenodesvaried = perfnodesvaried(bottomplanenodesindx,:);
                topplanevarieddists = zeros(size(topplanenodes,1),1);
                bottomplanevarieddists = zeros(size(bottomplanenodes,1),1);
                for j=1:length(topplanevarieddists)
                    topplanevarieddists(j,1) = norm(topplanenodesvaried(j,:)-mean(perfnodesvaried));
                end
                bottomplanevarieddists = zeros(size(bottomplanenodes,1),1);
                for j=1:length(bottomplanevarieddists)
                    bottomplanevarieddists(j,1) = norm(bottomplanenodesvaried(j,:)-mean(perfnodesvaried));
                end
                [topplanevarieddists,~] = sort(topplanevarieddists);
                [bottomplanevarieddists,~] = sort(bottomplanevarieddists);
                if length(topplanevarieddists) < length(bottomplanevarieddists)
                    distcomparison = abs(topplanevarieddists-bottomplanevarieddists(1:length(topplanevarieddists)));
                else
                    distcomparison = abs(topplanevarieddists(1:length(bottomplanevarieddists))-bottomplanevarieddists);
                end
                % Compare distances from mean point to nodes on both faces. For both faces, the distances should be very similar.
                [~,testvals]=find(distcomparison<planeztolerance);
                if length(testvals) > round(0.9*length(distcomparison))
                    % Plot top and bottom nodes to make sure comptutation is correct
                    figure()
                    plot3(perfnodes(:,1),perfnodes(:,2),perfnodes(:,3),'-k','LineWidth',2)
                    hold on
