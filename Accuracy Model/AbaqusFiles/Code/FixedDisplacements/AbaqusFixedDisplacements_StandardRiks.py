...
def assembly_and_pinBC():
    ...
    mdb.models['Model-1'].StaticStep(name='AlignmentStandard', previous='Initial', 
        maxNumInc=10000, nlgeom=ON)
    mdb.models['Model-1'].StaticRiksStep(name='AlignmentRiks', previous='AlignmentStandard', 
        maxLPF=1.0, maxNumInc=1000, initialArcInc=1e-05)
...
def apply_BCdisplacements(filename,stopratio):
    a = mdb.models['Model-1'].rootAssembly
    v = a.instances['impgeometry-1'].vertices
    with open(filename) as file:
        for line_num, line in enumerate(file,1):
            ...
            BCname2 = BCname + '_Riks'
            NODEname = str(line_list[7])
            # Find imperfect node in the assembly
            verts1 = v.findAt(coordinates=r1final)
            region1 = a.Set(vertices=verts1, name=NODEname)
            # Apply displacements in alignment step
            mdb.models['Model-1'].DisplacementBC(name=BCname, createStepName='AlignmentStandard', 
                region=region1, u1=dispx*stopratio, u2=dispy*stopratio, u3=dispz*stopratio, ur1=UNSET, ur2=UNSET, 
                ur3=UNSET, amplitude=UNSET, fixed=OFF, distributionType=UNIFORM, 
                fieldName='', localCsys=None)
            # Apply displacements in Riks step and deactivate previous BCs
            mdb.models['Model-1'].boundaryConditions[BCname].deactivate('AlignmentRiks')
            mdb.models['Model-1'].DisplacementBC(name=BCname2, createStepName='AlignmentRiks', 
                region=region1, u1=dispx*stopratio, u2=dispy*stopratio, u3=dispz*stopratio, ur1=UNSET, ur2=UNSET, 
                ur3=UNSET, amplitude=UNSET, fixed=OFF, distributionType=UNIFORM, 
                fieldName='', localCsys=None)
...
def setup_and_run_job():
    mdb.Job(name='TrussAlignDispStandardRiks', model='Model-1', description='', type=ANALYSIS, 
...
assembly_and_pinBC()
# apply_MPCconstraints("C:\\Users\\harsh\\Documents\\GitHub\\Bend-Forming\\Accuracy Model\\AbaqusFiles\\Code\\FixedDisplacements\\MPCconstraints.txt") % no MPC connectors with Riks?
apply_BCdisplacements("C:\\Users\\harsh\\Documents\\GitHub\\Bend-Forming\\Accuracy Model\\AbaqusFiles\\Code\\FixedDisplacements\\BCdisplacements.txt",0.5)
setup_and_run_job()
mdb.saveAs(pathName="C:\Users\harsh\Documents\GitHub\Bend-Forming\Accuracy Model\AbaqusFiles\TrussAlignDispStandardRiks") # save CAE file for future reference
# extract_RFforces("C:\\Users\\harsh\\Documents\\GitHub\\Bend-Forming\\Accuracy Model\\AbaqusFiles\\TrussAlignDispStandardRiks.odb", "C:\\Users\\harsh\\Documents\\GitHub\\Bend-Forming\\Accuracy Model\\AbaqusFiles\\Code\\FixedDisplacements\\RFatNodes.txt")
# extract_selfstress("C:\\Users\\harsh\\Documents\\GitHub\\Bend-Forming\\Accuracy Model\\AbaqusFiles\\TrussAlignDispStandardRiks.odb", "C:\\Users\\harsh\\Documents\\GitHub\\Bend-Forming\\Accuracy Model\\AbaqusFiles\\Code\\FixedDisplacements\\MaxMisesStress.txt")