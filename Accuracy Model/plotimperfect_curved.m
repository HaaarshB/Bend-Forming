function plotimperfect_curved(perfnodes,impnodes,curvenodestruct,perfgeombool,nodenumsbool)
%% Function to plot an imperfect Bend-Formed truss using coordinates of the perfect and imperfect nodes
    ...
    if isa(curvenodestruct,'struct')
        for i=1:(size(impnodes,1)-1)
            ...
            p1 = plot3(curvenodes(:,1),curvenodes(:,2),curvenodes(:,3),'r','LineWidth',2);
            hold on
        end
        startcurvenodes = curvenodestruct(1).coords;
        endcurvenodes = curvenodestruct(end).coords;
        scatter3([startcurvenodes(1,1), endcurvenodes(end,1)],[startcurvenodes(1,2), endcurvenodes(end,2)],[startcurvenodes(1,3), endcurvenodes(end,3)],75,'blue','filled');
    else
        p1 = plot3(impnodes(:,1),impnodes(:,2),impnodes(:,3),'-r','LineWidth',2);
        hold on
        scatter3([impnodes(1,1), impnodes(end,1)],[impnodes(1,2), impnodes(end,2)],[impnodes(1,3), impnodes(end,3)],75,'blue','filled');
    end
    if perfgeombool
        p2 = plot3(perfnodes(:,1),perfnodes(:,2),perfnodes(:,3),'-k','LineWidth',2);
